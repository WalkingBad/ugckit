...
Настройки в `config/default.yaml`:
...
audio:
  ...
  crossfade_ms: 100         # Кроссфейд между сегментами
  screencast_volume: 0.0    # Громкость скринкаста (0 = без звука)
```
...
| `--mode, -m` | Режим: `overlay` (по умолчанию) или `pip` |
| `--head-position` | Позиция головы для PiP режима |
